#
# Lab Environment for WSAA 2021.06 - M3
#
...
# Prepare the one Hyper-V server (SERVER2)

# Stop machine as it must be in power off state
Stop-VM -Name $VM3

# Prepare Hyper-V VM for nested virtualization
Set-VMMemory -VMName $VM3 -DynamicMemoryEnabled $false 
Set-VMProcessor -VMName $VM3 -ExposeVirtualizationExtensions $true
Get-VMNetworkAdapter -VMName $VM3 | Set-VMNetworkAdapter -MacAddressSpoofing On
