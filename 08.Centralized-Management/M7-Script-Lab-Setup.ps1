...
New-VM -Name $VM5 -MemoryStartupBytes 3072mb -VHDPath ($TargetFolder + $VM5 + ".vhdx") -Generation 2 -SwitchName "NAT vSwitch" | Set-VM -CheckpointType Production -AutomaticCheckpointsEnabled $false -PassThru | Set-VMMemory -DynamicMemoryEnabled $false
...
# Create VMs (with automatic checkpoints turned off and no dynamic memory)
New-VM -Name $VM6 -MemoryStartupBytes 6144mb -VHDPath ($TargetFolder + $VM6 + ".vhdx") -Generation 2 -SwitchName "NAT vSwitch" | Set-VM -CheckpointType Production -AutomaticCheckpointsEnabled $false -PassThru | Set-VMMemory -DynamicMemoryEnabled $false
New-VM -Name $VM7 -MemoryStartupBytes 2048mb -VHDPath ($TargetFolder + $VM7 + ".vhdx") -Generation 2 -SwitchName "NAT vSwitch" | Set-VM -CheckpointType Production -AutomaticCheckpointsEnabled $false -PassThru | Set-VMMemory -DynamicMemoryEnabled $false
