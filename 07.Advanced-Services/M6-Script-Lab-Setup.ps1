#
# Lab Environment for WSAA 2021.06 - M6
#
...
Invoke-Command -VMName $VM1 -Credential $LC -ScriptBlock { Install-WindowsFeature AD-Domain-Services, DNS, DHCP -IncludeManagementTools }
...
# Configure DHCP role
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Add-DhcpServerSecurityGroup }
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Restart-Service DHCPServer }
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Set-ItemProperty -Path registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\ServerManager\Roles\12 -Name ConfigurationState -Value 2 }
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Set-DhcpServerv4Binding -BindingState $true -InterfaceAlias "Ethernet" }
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Add-DhcpServerv4Scope -Name "Internal Network" -StartRange 192.168.99.100 -EndRange 192.168.99.200 -SubnetMask 255.255.255.0 }
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Set-DhcpServerv4OptionValue -OptionId 3 -value 192.168.99.1 }
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Set-DhcpServerv4OptionValue -OptionId 6 -value 192.168.99.2 }
Invoke-Command -VMName $VM1 -Credential $DC -ScriptBlock { Add-DhcpServerInDC }
